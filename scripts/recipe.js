...
module.exports = robot => {
  robot.hear(/^大将(!|！)献立 (.+)$/, async res => {
    ...
    try {
      const allResults = await Promise.all([getRecipesByKurashiru(query), getRecipesByCookpad(query).catch(error => [])]);
      const result = allResults.reduce((curr, result) => curr.concat(result), []).slice(0, 10);
...
async function getRecipesByKurashiru(query) {
  ...
  try {
    ...
    return $('.videos-list .video-list-content')
...
async function getRecipesByCookpad(query) {
  ...
  try {
    ...
    return $('.recipe-preview').map((index, node) => {
        const $node = $(node);
        return {
